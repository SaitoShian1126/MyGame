...
public:
	...
	static void SetScore(int score);	//スコアの設定
	static void SetScore(void);			//ランキングスコアの設定
	static void SetType(TYPE type);		//リザルトの種類の設定
	static CResult *Create();			//生成

private:
	static LPDIRECT3DTEXTURE9 m_pTextureResult[3];	//テクスチャへのポインタ
	static TYPE m_type;								//リザルトの種類
	static CNumber *m_pNumber[7];					//数字のポインタ
	static int m_nScore;							//スコア
	static int m_aData;								//データ
